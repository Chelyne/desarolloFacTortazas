...
export class AgregarProductoPage implements OnInit {
  ...
  ngOnInit() {
    console.log(this.sede, this.categoria);
  }

  createFormGroup() {
    return new FormGroup({
      // tslint:disable-next-line:max-line-length
      nombre: new FormControl('', [Validators.required, Validators.minLength(3), Validators.maxLength(35)]),
  ...
  agregarTallas() {
    ...
    if (this.tallas === undefined) {
      ...
      if (this.inputTalla && this.inputTalla.value.length > 0) {
    ...
    if (this.tallas.length < 10 && this.tallas.length > 0) {
      let existe = false;
      this.tallas.forEach(element => {
        if (this.inputTalla.value === element) {
